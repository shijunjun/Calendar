...
require_once __DIR__ . '/SolarTerm.php';
...
class Calendar
{
     ...
     * var array
     */
    private const HOLIDAY_SOLAR = [
        "0101" => "元旦","0214" => "情人节","0308" => "妇女节(1910)","0312" => "植树节(1979)",
        "0314" => "白色情人节","0315" => "国际消费者权益日(1983)","0318" => "全国科技人才活动日",
        "0401" => "愚人节","0421" => "全国企业家活动日(1994)","0426" => "世界知识产权日(2001)",
        "0501" => "劳动节(1889)","0504" => "青年节(1939)","0512" => "国际护士节",
        "0515" => "国际家庭日(1994)","0601" => "儿童节","0701" => "建党节(1921)",
        "0801" => "建军节(1927)","0806" => "国际电影节(1932)","0910" => "中国教师节(1985)",
        "1001" => "国庆节(1949)","1024" => "程序员节","1101" => "万圣节","1111" => "光棍节",
        "1201" => "世界艾滋病日","1224" => "平安夜","1225" => "圣诞节"
    ];
    /**
     * 阴历节假日
     * var array
     */
    private const HOLIDAY_LUNAR = [
        "0000" => "除夕", "0101" => "春节", "0115" => "元宵节", "0202" => "龙抬头&社日节&花朝节",
        "0303" => "上巳节", "0505" => "端午节", "0707" => "七夕节", "0715" => "中元节",
        "0815" => "中秋节", "0909" => "重阳节", "1001" => "寒衣节", "1208" => "腊八节", "1223" => "小年"
    ];
     ...
     * @return string
     */
    private function getHolidaySolar($year,$month,$day)
    {
        $key = "{$month}{$day}";
        if (isset(self::HOLIDAY_SOLAR[$key])){
            $holiday_solar = self::HOLIDAY_SOLAR[$key];
            $match = [];
            preg_match('/(?:\()(.*)(?:\))/i', $holiday_solar, $match);
            return (isset($match[1]) && $match[1] < $year) ? str_replace("{$match[0]}","",$holiday_solar) : $holiday_solar;
        }
        return "";
    }
    ...
    /**
     * 查询阴历节日
     * @param string $month 阴历月
     * @param string $day 阴历日
     */
    private function getHolidayLunar($year,$month,$day)
    {
        // 获取阴历节日
        if(!$holiday_lunar = isset(self::HOLIDAY_LUNAR["{$month}{$day}"]) ? self::HOLIDAY_LUNAR["{$month}{$day}"] : "")
        {
            $holiday_lunar = ($day == $this->lunarDays($year,$month) && $month==12) ? self::HOLIDAY_LUNAR["0000"] : "";
        }
    ...
    public function getTerms($year, $no = null)
    {

        $terms = SolarTerm::getSolarTerms($year);

        if ($no === null) return $terms;

        $no = (int)$no;
        if ($no < 1 || $no > 24) {
            throw new InvalidArgumentException("Parameter {\$no} error,  the range should be 1-24!");
        }

        array_walk($terms, function (&$item, $key) {
            $item = ["term" => $item['term_name'], 'date' => $item['date']];
        });
        if (!isset($terms[$no])) return null;

        return isset($terms[$no]) ? $terms[$no] : [];
    }
    ...
    public function getTerm($year, $no)
    {
        ...
        if (!($terms = $this->getTerms($year, $no))) return null;
        return (new \DateTime($terms['date']))->format("j");
    }
    ...
    public function solar2lunar($year, $month, $day, $hour = null)
    {
        ...
        $term = SolarTerm::getCurrentSolarTerm("{$year}-{$month}-{$day}");
        ...
        $lunarMonth = sprintf('%02d', $lunarMonth);
        $lunarDay = sprintf('%02d', $lunarDay);
        // 获取阳历节假日
        $holiday_solar = $this->getHolidaySolar($year, $month, $day);
        // 获取阴历节假日
        $holiday_lunar = $this->getHolidayLunar($lunarYear, $lunarMonth, $lunarDay);
        
        $holiday = $holiday_lunar . '&' . (isset($term['term_name']) ?  $term['term_name']."&" : "") .  $holiday_solar;
        ...
        return [
            ...
            'term' => trim($holiday,"&"),
            'term_date' => isset($term['date']) ? $term['date'] : "",
            'is_leap' => $isLeap,
        ];
    }
